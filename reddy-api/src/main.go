...
import (
	...
	"reddy-api/src/handlers"
	"reddy-api/src/helpers/databases"
	"reddy-api/src/types"
	"reddy-api/src/utils"
	"log"
	"net"
...
func main() {
	...
	app := fiber.New(fiber.Config{
		Prefork:      false,
		ServerHeader: "ManasaVaransi",
		AppName:      "ManasaVaransi v3.0",
	...
	urls := os.Getenv("URL")
	allowedOrigins := "http://localhost:8080,http://localhost:243"
	if urls != "" {
		allowedOrigins += "," + urls
	}
	...
	app.Use(func(c *fiber.Ctx) error {
		switch c.Path() {
		case "/login", "/hello", "/verify-user", "/verify-password", "/terminate-sessions":
			return c.Next()
		}
	...
	app.Use(func(c *fiber.Ctx) error {
		switch c.Path() {
		case "/hello", "/verify-user", "/verify-password", "/terminate-sessions":
			return c.Next()
		}
	...
	// 2-step auth endpoints for BhagyaSreeBorse frontend
	app.Post("/verify-user", func(c *fiber.Ctx) error {
		var body struct {
		...
		result, err := handlers.VerifyUser(body.Username)
		if err != nil {
			return err
		}
	...
	app.Post("/verify-password", func(c *fiber.Ctx) error {
		var body struct {
		...
		if err := c.BodyParser(&body); err != nil || body.Password == "" {
			return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": "Missing password"})
		}
		result, err := handlers.VerifyPassword(body.Identifier, body.Digest, body.Password, body.Email)
		if err != nil {
			return err
		}
		return c.JSON(result)
	...
	app.Post("/terminate-sessions", func(c *fiber.Ctx) error {
		var body struct {
		...
		result, err := handlers.TerminateSessions(body.FlowId, body.Identifier, body.Digest)
		if err != nil {
			return err
		}
		return c.JSON(result)
	...
	api.Get("/get", cache.New(cacheConfig), func(c *fiber.Ctx) error {
		...
		if err == nil && db != nil {
			cachedData, _ = db.FindByToken("goscrape", encodedToken)
		}
