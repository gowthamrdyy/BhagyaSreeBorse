...
const LastUpdatedContent = ({
  ...
  return (
    <div className="px-4 border-b border-border py-2 flex flex-col gap-2 ">
      <div className="w-full flex items-center justify-between ">
        <div className="h-full flex items-center justify-center">
          {query.isRefetching ? (
            <h1 className="text-foreground/50">Fetching</h1>
          ) : query.isLoading ? (
            <h1 className="text-foreground/50">Loading...</h1>
          ) : lastUpdated ? (
            <div className="flex gap-3 items-center justify-center">
              {!query.isError && (
                <span className="relative flex h-3 w-3 ">
                  <span className="absolute animate-ping inset-0 rounded-full bg-accent opacity-75"></span>
                  <span className="rounded-full h-2 w-2 bg-accent apply-inner-shadow-sm m-auto"></span>
                </span>
              )}
              <h1 className="text-foreground/50">Last updated {lastUpdated} ago </h1>
            </div>
          ) : (
            <h1 className="text-foreground/50">No data available</h1>
          )}
        </div>
      ...
      {query.isError && (
        <div className="bg-destructive/10 text-destructive  w-full justify-center items-center min-h-8 flex border border-destructive/20 rounded-lg text-sm ">
          {query.error.message}
        </div>
      )}
    </div>
